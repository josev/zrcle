class Comment < ActiveRecord::Base
  ...
  def self.get_comments(_params)
    if _params[:goal_id].present?
      Comment.joins("join user_goals on comments.user_id = user_goals.user_id and comments.goal_id = user_goals.goal_id").where(user_goals: {private: false, state: ["1","2"]}, goal_id: _params[:goal_id], to_user_id: nil)
